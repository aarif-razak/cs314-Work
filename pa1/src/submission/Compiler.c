...
{	
	...
	if((token != '=')){
		ERROR("assignment %c failure\n", token);
		exit(EXIT_FAILURE);
	}else{
		/*printf("poopoo\n");*/
		next_token();
		new = expr();
		CodeGen(STORE,initial, new, EMPTY_FIELD);
